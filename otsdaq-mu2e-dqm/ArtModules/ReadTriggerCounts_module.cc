...
namespace mu2e {
  ...
  void ReadTriggerCounts::analyze(const art::Event& event) {
    ...
    metricMan->sendMetric("TriggerCounts.TotalEvents", 1, "events", 2, artdaq::MetricMode::Accumulate | MetricMode::Rate););
    metricMan->sendMetric("TriggerCounts.TotalEventsCumulative", 1, "events", 2, artdaq::MetricMode::LastPoint | MetricMode::Rate););
    for (unsigned int i=0; i< trigNavig.getTrigPaths().size(); ++i){
      ...
      if(trigNavig.accepted(path)){
        ...
        metricMan->sendMetric("TriggerCounts."+path, 1, "events", 2, artdaq::MetricMode::Accumulate | MetricMode::Rate););
        metricMan->sendMetric("TriggerCounts."+path+"Cumulative", 1, "events", 2, artdaq::MetricMode::LastPoint | MetricMode::Rate););
	// Avoid double counting for total passed: has this event passed a trigger stream previously?
        if(hasNotPassedTrigger){
          ...
          metricMan->sendMetric("TriggerCounts.TotalPassedTrigger", 1, "events", 2, artdaq::MetricMode::Accumulate | MetricMode::Rate););
	  metricMan->sendMetric("TriggerCounts.TotalPassedTriggerCumulative", 1, "events", 2, artdaq::MetricMode::LastPoint | MetricMode::Rate););
